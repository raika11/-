 ...
 * @since 2020. 11. 8.
 */
public class ServiceMapRepositorySupportImpl extends QuerydslRepositorySupport implements
    ServiceMapRepositorySupport {
...
//        }
        ...
        if(searchType.equals("korname")) {
            ...
            if (len == 0){
                // service code 검색
                builder.and(serviceMap.masterCode.length().eq(2));
            } else if (len == 2) {
                // section code 검색
                builder.and(serviceMap.masterCode.length().eq(4)).and(serviceMap.masterCode.startsWith(keyword));
            } else {
                // content code 검색
                builder.and(serviceMap.masterCode.length().gt(4)).and(serviceMap.masterCode.startsWith(keyword));
            }
        }

//        Sort sort = Sort.by(Sort.Direction.DESC, "codeOrd");
        JPQLQuery<ServiceMap> query = queryFactory.selectFrom(serviceMap)
            .where(builder);
