...
public class GlobalExceptionHandler {
    ...
    public Object defaultException(HttpServletRequest request, HttpServletResponse response,
            Exception ex) throws IOException {
        logger.error("[defaultException] {}", ex);
        if (ex.getCause() != null && ex.getCause().getClass().equals(java.io.IOException.class)) {
            return null;
        } else {
            ...
            if (acceptHeader.contains("html")) {
                ...
                mav.addObject(TpsConstants.MODEL_ATTR_ROOTCAUSE,
                        rootCause == null ? ex : rootCause);
                return mav;
            } else {
                String message = McpString.isNullOrEmpty(ex.getMessage())
                        ? messageByLocale.get("wms.common.error", request) : ex.getMessage();
                return ResponseUtil.getErrorResponseEntity(response,
                        TpsConstants.HEADER_SERVER_ERROR, message);
            }
        }
    }
    ...
    public ResponseEntity<?> handleBindException(HttpServletRequest request,
        ...
        if (ex.getFieldErrorCount() > 0) {
            ...
            return ResponseUtil.getErrorResponseEntity(response, TpsConstants.HEADER_INVALID_DATA,
                    message, resultInvalidList);
        } else {
            return ResponseUtil.getErrorResponseEntity(response, TpsConstants.HEADER_INVALID_DATA,
                    ex.getMessage());
        }
    ...
    public ResponseEntity<?> handleConstraintViolationException(HttpServletRequest request,
        ...
        if (violations.size() > 0) {
            ...
            for (ConstraintViolation<?> violation : violations) {
                ...
                String argMessage = messageProp.replaceAll("\\{", "").replaceAll("\\}", "");
                ...
                invalidList
                        .add(new InvalidDataDTO(argName, messageByLocale.get(argMessage, request)));
            }
    ...
    public ResponseEntity<?> handleMethodArgumentNotValidException(HttpServletRequest request,
        ...
        if (bindingResult.getFieldErrorCount() > 0) {
            ...
            return ResponseUtil.getErrorResponseEntity(response, TpsConstants.HEADER_BAD_REQUEST,
                    ex.getMessage());
        }
    ...
    public ResponseEntity<?> handleMethodArgumentTypeMismatchException(HttpServletRequest request,
    ...
    public ResponseEntity<?> noDataException(HttpServletRequest request,
            HttpServletResponse response, NoDataException ex) {

        logger.error("[NoContentException] {}", ex);
        return ResponseUtil.getErrorResponseEntity(response, TpsConstants.HEADER_NO_DATA,
                ex.getMessage());
    }
    ...
    public ResponseEntity<?> invalidDataException(HttpServletRequest request,
            HttpServletResponse response, InvalidDataException ex) {
        ...
        if (invalidList != null && invalidList.size() > 0) {
            ...
            return ResponseUtil.getErrorResponseEntity(response, TpsConstants.HEADER_INVALID_DATA,
                    ex.getMessage(), resultInvalidList);
        } else {
            return ResponseUtil.getErrorResponseEntity(response, TpsConstants.HEADER_BAD_REQUEST,
                    ex.getMessage());
        }
    ...
    public ResponseEntity<?> ioException(HttpServletRequest request, HttpServletResponse response,
            IOException ex) {

        logger.error("[IOException] {}", ex);

        String message = "입출력에 오류가 있습니다. " + ex.getMessage();
        return ResponseUtil.getErrorResponseEntity(response, TpsConstants.HEADER_FILE_ERROR,
                message);
    }
    ...
    public ResponseEntity<?> templateParseException(HttpServletRequest request,
            HttpServletResponse response, TemplateParseException ex) {
        ...
        List<InvalidDataDTO> invalidList = new ArrayList<InvalidDataDTO>();
        invalidList.add(new InvalidDataDTO("syntax",
                TemplateParseException.ErrorMessage.getMessage(ex.getErrorCode()),
