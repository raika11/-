...
import {
    TemplateDialog,
    adSearchTypes as searchTypes,
    adSearchColumns as searchColumns
} from '../components';
import {
    clearAd as clear,
    getAds as getList,
...
const PageChildRelationAD = (props) => {
    ...
    useEffect(() => {
        if (latestDomainId && latestPageSeq) {
            const option = [
                ...
                { key: 'searchType', value: 'pageSeq' },
                { key: 'keyword', value: String(latestPageSeq) },
        ...
        (e) => {
            ...
            Object.keys(search).forEach((m) => {
                if (m === 'page') {
                    ...
                    const sortValue =
                        search.searchType === 'pageSeq' ? 'relOrder,asc' : 'adSeq,desc';
...
export default PageChildRelationAD;
