...
public class SkinHistRepositorySupportImpl extends QuerydslRepositorySupport
        implements SkinHistRepositorySupport {
    ...
    public Page<SkinHist> findList(HistSearchDTO search, Pageable pageable) {
        ...
        QueryResults<SkinHist> list = query.innerJoin(skinHist.skin, skin).fetchJoin()
                .innerJoin(skinHist.domain, domain).fetchJoin().where(builder).fetchResults();
