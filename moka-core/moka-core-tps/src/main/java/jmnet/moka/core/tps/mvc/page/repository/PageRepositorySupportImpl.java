 ...
 * @author jeon
 */
public class PageRepositorySupportImpl extends QuerydslRepositorySupport
        implements PageRepositorySupport {
    ...
    public List<Page> findByDomainId(String domainId) {
        ...
        JPQLQuery<Page> query =
                queryFactory.selectFrom(page).innerJoin(page.domain, domain).fetchJoin()
                        // .leftJoin(page.pageRelations, relation).fetchJoin()
                        .where(builder).orderBy(page.pageUrl.asc());
    ...
    public org.springframework.data.domain.Page<Page> findList(PageSearchDTO search,
        ...
        if (!McpString.isEmpty(searchType) && !McpString.isEmpty(keyword)) {
            if (searchType.equals("pageSeq")) {
                ...
                builder.and(page.pageSeq.like(keyword).or(page.pageName.contains(keyword))
        ...
        QueryResults<Page> list =
                query.innerJoin(page.domain, domain).fetchJoin().where(builder).fetchResults();
    ...
    // String keyword = search.getKeyword();
    //
    // if (McpString.isNotEmpty(search.getDomainId()) && !search.getDomainId().equals("all")) {
