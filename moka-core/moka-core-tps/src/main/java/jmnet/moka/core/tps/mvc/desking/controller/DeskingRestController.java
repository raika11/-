...
public class DeskingRestController {
    ...
    /**
     * 페이지의 모든 Work 초기화 및 Work 컴포넌트 목록 조회
     *
     ...
     */
    @GetMapping("/{areaSeq}")
    public ResponseEntity<?> getComponentWorkList(@PathVariable("areaSeq") @Min(value = 0, message = "{tps.area.error.min.areaSeq}") Long areaSeq, Principal principal)
            throws Exception {

        try {
            DeskingWorkSearchDTO search = DeskingWorkSearchDTO.builder().areaSeq(areaSeq).regId(principal.getName())
                .build();
...
//            }
            ...
            log.error("[FAIL TO LOAD DESKING WORK]", e);
            tpsLogger.error(ActionType.SELECT, "[FAIL TO LOAD DESKING WORK]", e, true);
...
//
//        } catch (Exception e) {
//            throw new Exception(messageByLocale.get("tps.desking.error.work.delete", request), e);
//        }
//    }
//
//    /**
