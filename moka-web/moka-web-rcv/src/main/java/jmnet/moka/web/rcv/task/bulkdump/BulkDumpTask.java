...
import jmnet.moka.web.rcv.common.object.JaxbObjectUnmarshaller;
...
import jmnet.moka.web.rcv.task.bulkdump.channel.BulkDumpClientChannel;
...
public class BulkDumpTask extends Task<DBTaskInputData> {
    ...
    protected void load(Node node, XMLUtil xu)
        ...
        if (McpString.isNullOrEmpty(bulkDumpEnvFile)) {
            throw new RcvException("bulkDumpEnvFile 환경 값 설정이 잘못되었습니다.");
        }
        ...
        try {
            ...
            if (!file.exists()) {
                throw new RcvException("bulkDumpEnvFile 파일을 찾을 수 없습니다.");
            }
            bulkDumpEnv = (BulkDumpEnv) JaxbObjectManager.getBasicVoFromXml(file, BulkDumpEnv.class);
            if( bulkDumpEnv == null )
                throw new RcvException("bulkDumpEnvFile 파일을 로드하는 중에 에러가 발생하였습니다. null");

            bulkDumpEnv.init();
        } catch (Exception e) {
            throw new RcvException("bulkDumpEnvFile 파일을 로드하는 중에 에러가 발생하였습니다.");
        }
    }
