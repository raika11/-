...
public class ActionLogger {
    ...
    private void init() {
        try {
            serverIp = InetAddress.getLocalHost() == null
                    ? ""
    ...
    private void log(String actor, ActionType actionType, ActionResult actionResult, long executedTime, String msg, Throwable t) {
        String clientIp = HttpHelper.getRemoteAddr();
        String actionMsg = McpString.defaultValue(msg) + (t != null ? t.getMessage() : "");
        log.info(LogMarker.ACTION, "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t", this.systemId, this.serverIp, clientIp, actor, actionType.code(),
                 actionResult.code(), executedTime, actionMsg);
    }
