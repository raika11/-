...
import jmnet.moka.web.rcv.task.bulkdump.process.basic.BulkProcessCommon;
 ...
 * Package : jmnet.moka.web.rcv.task.bulkdump.process
...
public class BulkJoongangProcess extends BulkProcessCommon<BulkJoongangArticle> {
    ...
    protected boolean doProcess_InsertUpdate(BulkJoongangArticle article, BulkDumpService dumpService) {
        ...
        String tmpContentHtml = RcvTagUtil.ripTagWithOrderRule(article.getContentHtml().toString(), "class=\"ab_photo photo_center", "px\"", 50 );
        tmpContentHtml = RcvTagUtil.ripTagWithOrderRule(tmpContentHtml, " wrappercss=\"photo_center\"", "px\"", 50 );
        tmpContentHtml = RcvTagUtil.ripTagWithOrderRule(tmpContentHtml, " wrappercss=\"photo_center\"", "px;\"", 50 );
        tmpContentHtml = RcvTagUtil.ripTagWithOrderRule(tmpContentHtml, "style=\"width:", "px\"", 50 );
        tmpContentHtml = RcvTagUtil.ripTagWithOrderRule(tmpContentHtml, "style=\"width:", "px;\"", 50 );
        tmpContentHtml = RcvTagUtil.ripTagWithOrderRule(tmpContentHtml, "wrapperwidth=", "px\"", 50 );
        tmpContentHtml = RcvTagUtil.ripTagWithOrderRule(tmpContentHtml, "wrapperwidth=", "px;\"", 50 );
        ...
        article.getContentHtml().setData(RcvTagUtil.ripTagWithOrderRule(article.getContentHtml().toString(), "<div class=\"tag_vod\"", "</div>"));  //동영상 제외
        article.getContentHtml().setData(RcvTagUtil.ripTagWithOrderRule(article.getContentHtml().toString(), "<div class=\"tag_audio\"", "</div>")); //오디오 제외
