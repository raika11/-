 ...
 * Project : moka
 * Package : jmnet.moka.core.tps.mvc.sns.service
 * ClassName : TwitterOAuthParams
 * Created : 2020-12-09 ince
 * </pre>
 *
 * @author ince
 * @since 2020-12-09 16:05
 */
public class TwitterOAuthParams {
    ...
    /**
     * OAuth1Params is mostly just a container for OAuth 1.0a parameters. The token is optional to allow for OAuth 1.0 two-legged requests.
     */
    public static class OAuth1Params {
        ...
        public OAuth1Params(String token, String consumerKey, String nonce, Long timestampSecs, String timestampStr, String signature,
                String signatureMethod, String version) {
            this.token = token;
        ...
        public String toString() {
            return String.format("%s=%s,%s=%s,%s=%s,%s=%s(->%s),%s=%s,%s=%s,%s=%s", OAUTH_TOKEN, valueOrUnset(token), OAUTH_CONSUMER_KEY,
                    valueOrUnset(consumerKey), OAUTH_NONCE, valueOrUnset(nonce), OAUTH_TIMESTAMP, timestampStr, timestampSecs, OAUTH_SIGNATURE,
                    valueOrUnset(signature), OAUTH_SIGNATURE_METHOD, valueOrUnset(signatureMethod), OAUTH_VERSION, valueOrUnset(version));
        }
    }


    /**
     * A collector for OAuth and other params. There are convenience methods for determining if it has all OAuth parameters set, just the token set,
     * and for obtaining a list of all params for use in producing the normalized request.
     */
    public static class OAuthParamsBuilder {
        ...
        private Map<String, String> paramsHandler = new HashMap<>();
        private Map<String, String> otherOAuthParamsHandler = new HashMap<>();
        ...
        // effeciency when printing the class for debugging
        public String toString() {
            return String.format("%s=%s,%s=%s,%s=%s,%s=%s,%s=%s(->%s),%s=%s,%s=%s,%s=%s", BEARER_TOKEN, valueOrUnset(v2Token), OAUTH_TOKEN,
                    valueOrUnset(token), OAUTH_CONSUMER_KEY, valueOrUnset(consumerKey), OAUTH_NONCE, valueOrUnset(nonce), OAUTH_TIMESTAMP,
                    timestampStr, timestampSecs, OAUTH_SIGNATURE, valueOrUnset(signature), OAUTH_SIGNATURE_METHOD, valueOrUnset(signatureMethod),
        ...
        public boolean isOAuth1TwoLegged() {
            return (token == null || "".equals(token)) && consumerKey != null && nonce != null && timestampStr != null && signature != null
        ...
        public OAuth1Params oAuth1Params() {
            return new OAuth1Params(token, consumerKey, nonce, timestampSecs, timestampStr, signature, signatureMethod, version);
        }
    }
    ...
    public interface OAuthParamsHelper {
        /**
         * allows one to override the default behavior when parsing timestamps, which is to parse them as integers, and ignore timestamps that are
         * malformed
         */
    ...
    /**
     * Provides the default implementation of the OAuthParamsHelper trait Though stateless and threadsafe, this is a class rather than an object to
     * allow easy access from Java. Scala codebases should use the corresponding STANDARD_OAUTH_PARAMS_HELPER object instead.
     */
