 ...
 * @author kspark
 */
public class CpMerger extends AbstractElementMerger {

	private static final Logger logger = LoggerFactory.getLogger(CpMerger.class);
	
	public CpMerger(TemplateMerger<?> templateMerger) throws IOException {
	...
	public void merge(TemplateElement element, MergeContext context, StringBuilder sb) throws TemplateMergeException {
		...
		try {
			TemplateRoot templateRoot = templateMerger.getParsedTemplate(ITEM_COMPONENT, element.getAttribute(ATTR_ID));
