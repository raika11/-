  ...
  try {
    ...
    try {
      return require.resolve(
        path.join(
          process.cwd(),
          "node_modules/@moka/monaco-editor/esm",
          filePath
        )
      );
    } catch (err) {
...
class MonacoEditorWebpackPlugin implements webpack.Plugin {
  ...
  constructor(options: IMonacoEditorWebpackPluginOpts = {}) {
    ...
    this.options = {
      languages: coalesce(languages.map((id) => languagesById[id])).concat(
        customLanguages
      ),
...
function createLoaderRules(
  ...
  const workerPaths = fromPairs(
    workers.map(({ label, entry }) => [
      label,
      getWorkerFilename(filename, entry),
    ])
  );
  ...
  return [
    {
      test: /@moka[/\\]monaco-editor[/\\]esm[/\\]vs[/\\]editor[/\\]editor.(api|main).js/,
      use: [
        {
