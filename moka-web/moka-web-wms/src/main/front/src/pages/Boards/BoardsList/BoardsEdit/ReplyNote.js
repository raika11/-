 ...
 */
const ReplyNote = ({ data, onChangeFormData, jpodBoardId }) => {
    const dispatch = useDispatch();
    const { boardId } = useParams();
    const contentsReply = useSelector((store) => store.board.listMenu.contents.reply);
    const jpodReply = useSelector((store) => store.jpod.jpodNotice.reply);
        ...
        dispatch(
            uploadBoardContentsImage({
                ...
                callback: ({ header, body }) => {
                    if (header.success) {
                        let tempContent = `${contentsReply.content || jpodReply.content} <img src="${body}">`;
