...
imageEditor.on({
    objectAdded: function(objectProps) {
        window.console.info(objectProps);
    },
    ...
    addText: function(pos) {
        imageEditor
            ...
            .then(function(objectProps) {
                window.console.log(objectProps);
            });
    },
...
$btnFlipX.on('click', function() {
    imageEditor.flipX().then(function(status) {
        window.console.log('flipX: ', status.flipX);
        window.console.log('flipY: ', status.flipY);
        window.console.log('angle: ', status.angle);
    });
});
...
*/
$inputCheckBlur.on('blur', function() {
    applyOrRemoveFilter(this.checked, 'blur', {
        blocksize: parseInt($inputRangeBlurValue.val(), 10),
    });
});
