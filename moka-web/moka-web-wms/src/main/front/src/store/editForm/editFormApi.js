...
export const getEditFormList = ({search}) => {
  return instance.get(`/api/edit-forms?${qs.stringify(search)}`).catch(
      (err) => {
        throw err;
      });
};
...
export const postEditForm = ({formSeq, itemSeq, formData}) => {
  return instance.post(`/api/edit-forms/${formSeq}/items/${itemSeq}`,
      qs.stringify(formData)).catch((err) => {
    throw err;
  });
};
...
export const putEditForm = ({formSeq, itemSeq, formData}) => {
  return instance.put(`/api/edit-forms/${formSeq}/items/${itemSeq}`,
      qs.stringify(formData)).catch((err) => {
    throw err;
  });
};
