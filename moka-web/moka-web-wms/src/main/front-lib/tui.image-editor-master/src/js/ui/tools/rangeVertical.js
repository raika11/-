...
class RangeVertical {
    ...
    constructor(rangeElements, options = {}) {
        ...
        this.rangeHeight = this._getRangeHeight();
    ...
    set value(value) {
        ...
        let bottomPosition = (absValue * this.rangeHeight) / this._absMax;

        if (this.rangeHeight < bottomPosition) {
            bottomPosition = this.rangeHeight;
        }

        this.pointer.style.bottom = `${bottomPosition}px`;
        this.subbar.style.top = `${this.rangeHeight - bottomPosition}px`;
     ...
     */
    _getRangeHeight() {
        const getElementHeight = element => toInteger(window.getComputedStyle(element, null).height);

        return getElementHeight(this.rangeElement) - getElementHeight(this.pointer);
    }
     ...
     */
    _drawRangeElement() {
        this.rangeElement.classList.add('tui-image-editor-vertical-range');

        this.bar = document.createElement('div');
        this.bar.className = 'tui-image-editor-virtual-vertical-range-bar';

        this.subbar = document.createElement('div');
        this.subbar.className = 'tui-image-editor-virtual-vertical-range-subbar';

        this.pointer = document.createElement('div');
        this.pointer.className = 'tui-image-editor-virtual-vertical-range-pointer';
     ...
     */
    _changeSlide(event) {
        const changePosition = -event.screenY;
        const diffPosition = changePosition + this.firstPosition;
        let touchPy = this.firstBottom + diffPosition;
        touchPy = touchPy > this.rangeHeight ? this.rangeHeight : touchPy;
        touchPy = touchPy < 0 ? 0 : touchPy;

        this.pointer.style.bottom = `${touchPy}px`;
        this.subbar.style.top = `${this.rangeHeight - touchPy}px`;

        const ratio = touchPy / this.rangeHeight;
    ...
    _changeSlideFinally(event) {
        ...
        if (event.target.className !== 'tui-image-editor-vertical-range') {
            return;
        }
        const touchPy = event.offsetY;
        const ratio = touchPy / this.rangeHeight;
        const value = this._absMax * ratio + this._min;
        this.pointer.style.bottom = `${ratio * this.rangeHeight}px`;
        this.subbar.style.top = `${(1 - ratio) * this.rangeHeight}px`;
        this.value = value;
    ...
    _startChangingSlide(event) {
        this.firstPosition = event.screenY;
        this.firstBottom = toInteger(this.pointer.style.bottom) || 0;
...
snippet.CustomEvents.mixin(RangeVertical);
