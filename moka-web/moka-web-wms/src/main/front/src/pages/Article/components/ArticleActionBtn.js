...
const ArticleActionBtn = forwardRef(({ data: initialData }, ref) => {
    ...
    return (
        <div className="d-flex align-items-center h-100">
            <Button size="sm" variant="outline-table-btn2" className="mr-1 px-1 flex-shrink-0" onClick={() => data.handleClickDelete(data)}>
                삭제
            </Button>
            <Button size="sm" variant="outline-table-btn2" className="mr-1 px-1 flex-shrink-0" disabled={data.serviceFlag !== 'Y'} onClick={() => data.handleClickStop(data)}>
                중지
            </Button>
