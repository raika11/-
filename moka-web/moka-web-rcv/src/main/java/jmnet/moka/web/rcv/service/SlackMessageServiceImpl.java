...
import jmnet.moka.web.rcv.service.vo.SlackMessageVo;
...
public class SlackMessageServiceImpl implements SlackMessageService {
    ...
    @Value("${rcv.slack.timer.firstlimitcount}")
    private long GAP_TIME_COUNT;

    @Value("#{timeHumanizer.parse('${rcv.slack.timer.waitingtime}', 30*1000)}")
    private long GAP_TIME_START;

    @Value("#{timeHumanizer.parse('${rcv.slack.timer.limitwaitingtime}', 30*1000*1000)}")
    private long GAP_TIME_LIMIT;

    @Value("#{timeHumanizer.parse('${rcv.slack.timer.pausetime}', 60*1000*1000)}")
    private long GAP_TIME_PAUSE;

    @Value("${rcv.slack.channelname}")
