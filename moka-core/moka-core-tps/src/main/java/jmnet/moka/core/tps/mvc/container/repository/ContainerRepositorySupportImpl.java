 ...
 *
 */
public class ContainerRepositorySupportImpl extends QuerydslRepositorySupport
        implements ContainerRepositorySupport {
    ...
    public Page<Container> findRelList(RelSearchDTO search, Pageable pageable) {
        ...
        JPQLQuery<Container> query = queryFactory.selectFrom(container).where(
                container.containerSeq.in(JPAExpressions.select(containerRel.container.containerSeq)
                        .from(containerRel).where(builder)));
