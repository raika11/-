 ...
 * @author kspark
 */
public class TpMerger extends AbstractElementMerger {

	private static final Logger logger = LoggerFactory.getLogger(TpMerger.class);
	
	public TpMerger(TemplateMerger<?> templateMerger) throws IOException {
	...
	public void merge(TemplateElement element, MergeContext context, StringBuilder sb) throws TemplateMergeException  {
		...
		try {
			TemplateRoot templateRoot = templateMerger.getParsedTemplate(ITEM_TEMPLATE, element.getAttribute(ATTR_ID));
