...
function getBasicLanguages() {
  return new Promise((resolve, reject) => {
    glob('./node_modules/@moka/monaco-editor/esm/vs/basic-languages/*/*.contribution.js', { cwd: path.dirname(__dirname) }, (err, files) => {
      if (err) {
        reject(err);
        return;
      }
      ...
      resolve(files.map((file) => {
        const entry = file.substring('./node_modules/@moka/monaco-editor/esm/'.length).replace(/\.js$/, '');
...
function readAdvancedLanguages() {
  return new Promise((resolve, reject) => {
    glob('./node_modules/@moka/monaco-editor/esm/vs/language/*/monaco.contribution.js', { cwd: path.dirname(__dirname) }, (err, files) => {
      if (err) {
        reject(err);
        return;
      }
      ...
      resolve(
        files
          .map(file => file.substring('./node_modules/@moka/monaco-editor/esm/vs/language/'.length))
...
function getAdvancedLanguages() {
  ...
  function checkFileExists(moduleName) {
    const filePath = path.join(__dirname, '..', 'node_modules/@moka/monaco-editor/esm', `${moduleName}.js`);
...
function generateFeatures() {
  ...
  let features = [];
  const files = (
    fs.readFileSync(path.join(__dirname, '../node_modules/@moka/monaco-editor/esm/vs/editor/edcore.main.js')).toString()
    + fs.readFileSync(path.join(__dirname, '../node_modules/@moka/monaco-editor/esm/vs/editor/editor.all.js')).toString()
  );
  files.split(/\r\n|\n/).forEach(line => {
