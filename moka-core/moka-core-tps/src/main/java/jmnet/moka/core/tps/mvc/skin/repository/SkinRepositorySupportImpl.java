...
public class SkinRepositorySupportImpl extends QuerydslRepositorySupport implements SkinRepositorySupport {
    ...
    public Page<Skin> findList(SkinSearchDTO search, Pageable pageable) {
        ...
        if (!McpString.isEmpty(searchType) && !McpString.isEmpty(keyword)) {
            if (searchType.equals("skinSeq")) {
                ...
                builder.and(skin.skinSeq.like(keyword)
                                        .or(skin.skinName.contains(keyword)));
            }
        }

        JPQLQuery<Skin> query = queryFactory.selectFrom(skin);
        query = getQuerydsl().applyPagination(pageable, query);
        QueryResults<Skin> list = query.innerJoin(skin.domain, domain)
                                       .fetchJoin()
                                       .where(builder)
                                       .fetchResults();
