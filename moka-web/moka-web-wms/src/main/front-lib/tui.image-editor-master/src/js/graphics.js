...
import MosaicDrawingMode from './drawingMode/mosaic';
...
class Graphics {
     ...
     * @param {String} mode Can be one of <I>'CROPPER', 'FREE_DRAWING', 'LINE', 'TEXT', 'SHAPE', 'MOSAIC'</I>
    ...
    /**
     * Get cropped rect
     * @param {number} [width] cropzone rect width
     * @param {number} [height] cropzone rect height
     */
    setCustomCropzoneRect(width, height) {
        this.getComponent(components.CROPPER).setCustomCropzoneRect(width, height);
    }
    ...
    /**
     * Set states of current drawing Mosaic
    ...
    setDrawingMosaic(type, options) {
        this.getComponent(components.MOSAIC).setStates(type, options);
    }
     ...
     */
    resizeCanvasImage(widht, height) {
        this.getComponent(components.RESIZE).resizeCanvasImage(widht, height);
    }
    ...
    _createComponents() {
        ...
        this._register(this._componentMap, new Mosaic(this));
     ...
     * @param {number} x - set position
     * @param {number} y - set position
     * @private
     */
    _callbackAfterLoadingImageObject(obj, x, y, callback) {
