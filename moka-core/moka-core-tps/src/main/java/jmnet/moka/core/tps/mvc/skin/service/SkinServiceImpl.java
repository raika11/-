...
public class SkinServiceImpl implements SkinService {
    ...
    private void insertRel(Skin skin)
        ...
        List<ParsedItemDTO> parsedItemDTOList = TemplateParserHelper.getItemList(skin.getSkinBody());
    ...
    public void updateRelItems(Component newComponent, Component orgComponent) {
        ...
        if (updateDS) {
            ...
            if (!newComponent.getDataType()
                ...
                List<SkinRel> relList = skinRelRepository.findList(MokaConstants.ITEM_COMPONENT, newComponent.getComponentSeq());

                for (SkinRel rel : relList) {
                    SkinRel newRel = SkinRel.builder()
                                            .skin(rel.getSkin())
    ...
    @Override
    public Optional<SkinHist> findSkinHistBySeq(Long histSeq) {
        return skinHistRepository.findById(histSeq);
    }
