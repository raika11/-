 ...
 * @param {number} [options.title] - editor title
 * @param {number} [options.lang] - editor language
 ...
 *  @param {Object} [options.resize] - image resize value
 *  @param {Object} [options.crop] - image crop size value
...
class ImageEditor {
    ...
    _onKeyDown(e) {
        ...
        if (isRemoveReady && isDeleteKey && document.activeElement.tagName !== 'INPUT') {
            e.preventDefault();
     ...
     * });
     */
    addImageObject(imgUrl, type) {
        if (!imgUrl) {
            return Promise.reject(rejectMessages.invalidParameters);
        }

        return this.execute(commands.ADD_IMAGE_OBJECT, imgUrl, type);
    }
    ...
    /**
     * resize image
     * @param {Object} imageSize imageSize
     *  @param {Number} width image width
     *  @param {Number} height image height
     * @example
     * imageEditor.resize(imageEditor.getResizeImageSize());
     */
    resizeCanvasImage(width, height) {
        ...
        this.loadImageFromURL(canvas.toDataURL(), this.getImageName()).then(sizeValue => {
    ...
    setCustomCropzoneRect(width, height) {
        this._graphics.setCustomCropzoneRect(width, height);
    }

    /**
     * Set the cropping rect
     * @param {number} [width] crop rect mode [1, 1.5, 1.3333333333333333, 1.25, 1.7777777777777777]
     */
    ...
    setDrawingMosaic(type, options) {
        this._graphics.setDrawingMosaic(type, options);
    }
    /**
     * Add shape
     * @param {string} type - Shape type (ex: 'rect', 'circle', 'triangle')
    ...
    addMosaic(type, options) {
        options = options || {};

        this._setPositions(options);

        return this.execute(commands.ADD_MOSAIC, type, options);
    }
     ...
     *      @param {number} [options.rx] - Radius x value (When type option is 'circle', this options can use)
     *      @param {number} [options.ry] - Radius y value (When type option is 'circle', this options can use)
     *      @param {boolean} [options.isRegular] - Whether resizing mosaic has 1:1 ratio or not
     * @param {boolean} isSilent - is silent execution or not
     * @returns {Promise}
     */
    changeMosaic(id, options, isSilent) {
        const executeMethodName = isSilent ? 'executeSilent' : 'execute';

        return this[executeMethodName](commands.CHANGE_MOSAIC, id, options);
    }

    changeWatermarkOpacity(id, options) {
        const executeMethodName = 'execute';
        return this[executeMethodName](commands.CHANGE_WATERMARK_OPACITY, id, options);
    }
