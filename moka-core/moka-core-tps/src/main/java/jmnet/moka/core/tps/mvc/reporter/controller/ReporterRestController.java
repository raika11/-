...
public class ReporterRestController {
    ...
    @GetMapping
    public ResponseEntity<?> getReporterMgrList(@SearchParam SearchDTO search, @RequestAttribute Long processStartTime) {

        // 조회
        Page<Reporter> returnValue = reporterService.findAllReporterMgr(search);

        // 리턴값 설정
        ResultListDTO<ReporterSimpleDTO> resultListMessage = new ResultListDTO<>();
        List<ReporterSimpleDTO> reporterDTOList = modelMapper.map(returnValue.getContent(), ReporterSimpleDTO.TYPE);
