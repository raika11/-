...
public class RestTemplateHelper {
    ...
    //response 데이터가 XML로 전달되어 파싱이 어려운 관계로 JSON으로 데이터 선언 후 처리
    /**
     * <pre>
     * RestTemplate을 이용하여 외부 서버와 데이터를 주고 받는다.
     * MultiValueMap에 파라미터를 담아 전달
     ...
     */
    public ResponseEntity<JSONArray> getJson(String url, MultiValueMap<String, Object> params, MultiValueMap<String, String> headers) {

        return requestJson(url, HttpMethod.GET, params, headers);
    }
     ...
     * @return ResponseEntity
     */
    private ResponseEntity<JSONArray> requestJson(String url, HttpMethod method, MultiValueMap<String, Object> params,
                                                  MultiValueMap<String, String> headers) {
        ...
        String uuid = UUID
                ...
                .toString();
        log.info("External System Request Start : txid : {}, url : {}, params : {}", uuid, url, params);
        ResponseEntity<JSONArray> responseBody = restTemplate.exchange(requestEntity, JSONArray.class);
        log.info("External System Request end : txid : {}", uuid);
        return responseBody;
    }
}
