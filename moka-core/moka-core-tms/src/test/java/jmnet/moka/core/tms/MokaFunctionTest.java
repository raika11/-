...
public class MokaFunctionTest {
    ...
    public static void opvTimeTest() {
        MokaFunctions mf = new MokaFunctions();
        String[] ovpTimes = {"4525111", "3600111", "1559111", "60111", "202111", "69111", "32111"};
        for ( String ovpTime : ovpTimes) {
            System.out.println(ovpTime+" --> "+mf.ovpTime(ovpTime));
        }
    }

    public static void agoTimeTest() {
        MokaFunctions mf = new MokaFunctions();
        ...
        String sec = LocalDateTime
                .now()
                .minusSeconds(randSec)
                .format(MokaConstants.dtf);
        String min = LocalDateTime
                .now()
                .minusMinutes(randMin)
                .format(MokaConstants.dtf);
        String hour = LocalDateTime
                .now()
                .minusHours(randHour)
                .format(MokaConstants.dtf);
        String yesterday = LocalDateTime
                .now()
                .minusDays(1)
                .minusHours(1)
                .format(MokaConstants.dtf);
        String lastYear = LocalDateTime
                .now()
                .minusYears(1)
                .format(MokaConstants.dtf);
        String dateTimes =
                "sec=" + sec + "," + "min=" + min + "," + "hour=" + hour + "," + "day=" + yesterday + "," + "s=" + yesterday + ",e=" + now + ","
                        + "s1=" + lastYear + ",e2=" + now;
