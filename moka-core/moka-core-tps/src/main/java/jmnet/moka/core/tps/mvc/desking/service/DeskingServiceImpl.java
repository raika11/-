...
import jmnet.moka.core.tps.mvc.component.service.ComponentWorkService;
...
import jmnet.moka.core.tps.mvc.desking.repository.DeskingRelHistRepository;
import jmnet.moka.core.tps.mvc.desking.repository.DeskingRelRepository;
import jmnet.moka.core.tps.mvc.desking.repository.DeskingRelWorkRepository;
...
public class DeskingServiceImpl implements DeskingService {
    ...
    @Autowired
    private DeskingRelRepository deskingRelRepository;

    @Autowired
    private DeskingHistRepository deskingHistRepository;

    @Autowired
    private DeskingRelHistRepository deskingRelHistRepository;

    @Autowired
    private DeskingWorkRepository deskingWorkRepository;

    @Autowired
    private DeskingRelWorkRepository deskingRelWorkRepository;
...
//        return cnt > 0 ? true : false;
    ...
    public void InitComponentWorks(Long pageSeq, String creator) {
        // 기존의 작업용 데이타 삭제
        componentWorkMapper.deleteByCreator(creator);
        deskingWorkMapper.deleteRelByCreator(creator);
        deskingWorkMapper.deleteByCreator(creator);

        // 페이지의 수동컴포넌트를 작업자용 컴포넌트로 일괄 등록
        WorkSearchDTO search = WorkSearchDTO.builder().pageSeq(pageSeq).creator(creator)
                .createYmdt(McpDate.nowStr()).build();
        componentWorkMapper.bulkInsert(search);

        // 페이지의 편집기사를 작업자용 편집기사로 일괄 등록
        deskingWorkMapper.importDeskingWorks(search);
        deskingWorkMapper.importDeskingRelWorks(search);
    }
    ...
    public List<DeskingComponentWorkVO> getComponentWorkAll(Long pageSeq, Long editionSeq,
        ...
        if (componentList.size() > 0) {
            List<DeskingWorkVO> deskingAllList = deskingWorkMapper.findDeskingWorkAll(search);
            List<DeskingRelWorkVO> deskingRelAllList =
                    deskingWorkMapper.findDeskingRelWorkAll(search);

            for (int i = 0; i < componentList.size(); i++) {
                // 편집기사
                Long datasetSeq = componentList.get(i).getDatasetSeq();
                Long componentSeq = componentList.get(i).getComponentSeq();
                List<DeskingWorkVO> deskingList = deskingAllList.stream().filter(d -> {
                    return d.getDatasetSeq().equals(datasetSeq);
                }).sorted(Comparator.comparingInt(DeskingWorkVO::getContentsOrder))
                        .collect(Collectors.toList());
                for (int idx = 0; idx < deskingList.size(); idx++) {
                    deskingList.get(idx).setComponentSeq(componentSeq);
                }
                componentList.get(i).setDeskingWorks(deskingList);

                // 관련편집기사
                List<DeskingRelWorkVO> deskingRelList = new ArrayList<DeskingRelWorkVO>();
                for (int j = 0; j < deskingList.size(); j++) {
                    Long parentDeskingSeq = deskingList.get(j).getDeskingSeq();
                    for (int k = 0; k < deskingRelAllList.size(); k++) {
                        if (deskingRelAllList.get(k).getDeskingSeq().equals(parentDeskingSeq)) {
                            DeskingRelWorkVO vo = ResourceMapper.getDefaultObjectMapper()
                                    .convertValue(deskingRelAllList.get(k), DeskingRelWorkVO.class);
                            deskingRelList.add(vo);
                        }
                    }
                    List<DeskingRelWorkVO> sortList = deskingRelList.stream()
                            .sorted(Comparator.comparingInt(DeskingRelWorkVO::getRelOrder))
                            .collect(Collectors.toList());
                    componentList.get(i).setDeskingRelWorks(sortList);
                }
